<?php
/**
  * @package cssecoST
  */
?>
<section id="section1" class="products_section1 eyecover">
  <div class="container">
    <div class="row">
      <div class="col-12 col-md-6 d-md-flex flex-column justify-content-between">
        <h1 class="bull"><span>Hand senitaizer</span></h1>
        <p>Your wish is our commend, <br> GGMT is proud to be a one-stop-shop for any PPE product needs.</p>
        <p>If you need it, we will get it.</p>
        <div class="cssecoBtnWrapper">
          <a href="<?php echo home_url() ?>/contact" class="cssecoBtn"><span>Start today</span></a>
        </div>
      </div>
      <div class="col-12 col-md-6 d-md-flex align-items-center justify-content-center">
      <img src="<?php echo get_template_directory_uri() ?>/dist/imgs/products/sanitizer/section1/1.png" alt="">
      </div>
    </div>
  </div>
</section>